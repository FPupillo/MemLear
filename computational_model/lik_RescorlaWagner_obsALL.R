lik_RescorlaWagner_obsALL<-function (Data,alpha, beta,print,  sceneNum){
...
#   sceneNum: how many scenes
#
# Output:
#   Negative Log Likelihood
# -------------

initialQ<-matrix(0, ncol=sceneNum, nrow=sceneNum)
# assign trialNumber: it is the length of the row of the data file
Data$trialN<-rep(1:(length(Data[,1])/sceneNum), times=sceneNum)
...
Qindex<-initVariab(sceneNum, Data)$Qindex
Pindex<-initVariab(sceneNum, Data)$Pindex
Deltaindex<-initVariab(sceneNum, Data)$Deltaindex

# Counter for indicating which scene has to be updated
count<-rep(0, sceneNum)
...
for (t in 1: nrow(Data)){
  ...
  # prior update
    Data$Qupdate[t]<-unlist( Qnew [Data$response[t]]- Q [Data$response[t]])
    
    if (Data$trialN[t]>1){
    Data$Pupdate[t]<-unlist( p [Data$response[t-1]]- Data[t-1, Pindex][Data$response[t-1]])
    Data$Deltatminus1[t]<-Data$Delta[t-1]
  ...
  Data$Delta[t]<-unlist(delta[Data$response[t]])
}
